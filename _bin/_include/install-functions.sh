...
wpi_handle_composer()
{
    ...
    then
        ...
        cp "$PROVISIONDIR/composer.json" "$INSTALL_PATH";
...
wpi_handle_gitignore()
{
    ...
    then
        ...
        echo "$PROVISIONDIR/.gitignore";
        cp "$PROVISIONDIR/.gitignore" "$INSTALL_PATH";
    fi
...
wpi_test_url()
{
    ...
    if [ "$CHECK_URL" = 404 ]; then
        ...
        CONTINUE=$(ask "Contionue installation ? y/N");
        if [ "$CONTINUE" = "y" ]; then
            echo "✔️ Processing installation";
        else
            exit;
        fi
    else
